...
const gtfsRide = async initialConfig => {
  ...
  try {
    ...
    const boardAlightCount = db.prepare('SELECT count(*) from board_alight').get();
    const oldestDate = db.prepare('SELECT service_date from board_alight ORDER BY service_date ASC LIMIT 1').get();
    const newestDate = db.prepare('SELECT service_date from board_alight ORDER BY service_date DESC LIMIT 1').get();
    const stopsWithDataResults = db.prepare('SELECT DISTINCT stop_id from board_alight').all();
    const stopsWithData = stopsWithDataResults.map(stop => stop.stop_id);
    const stopsWithoutData = db.prepare(`SELECT DISTINCT stop_id, stop_name from stops where stop_id NOT IN (${stopsWithData.map(() => '?').join(',')})`).all(...stopsWithData);
    const tripsWithDataResults = db.prepare('SELECT DISTINCT trip_id from board_alight').all();
    const tripsWithData = tripsWithDataResults.map(trip => trip.trip_id);
    const tripsWithoutData = db.prepare(`SELECT DISTINCT trips.trip_id, trips.direction_id, trips.route_id, routes.route_short_name, routes.route_long_name from trips INNER JOIN routes ON routes.route_id = trips.route_id where trips.trip_id NOT IN (${tripsWithData.map(() => '?').join(',')})`).all(...tripsWithData);
