...
module.exports = async initialConfig => {
  ...
  const stopsWithoutData = await db.all(`SELECT DISTINCT stop_id, stop_name from stops where stop_id NOT IN (${stopsWithData.map(stop => '?').join(',')})`, stopsWithData);
  ...
  const tripsWithoutData = await db.all(`SELECT DISTINCT trips.trip_id, trips.direction_id, trips.route_id, routes.route_short_name, routes.route_long_name from trips INNER JOIN routes ON routes.route_id = trips.route_id where trips.trip_id NOT IN (${tripsWithData.map(trip => '?').join(',')})`, tripsWithData);
