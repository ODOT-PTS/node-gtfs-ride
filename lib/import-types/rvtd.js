...
export const formatRVTDLine = ({ record, lineNumber }) => {
  ...
  const routes = getRoutes({ route_short_name: routeShortName }, ['route_id']);
  ...
  const matchedTrip = findTripByFirstStoptime({ routeId: routes[0].route_id, stopId: matchedStop.stop_id, stopSequence: record.stope_sequence }, record.service_arrival_date, gtfsTime);
