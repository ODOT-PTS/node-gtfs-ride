...
export const formatLTDLine = ({ record, lineNumber }) => {
  const stops = getStops({ stop_id: record.stop }, ['stop_id']);

  if (stops.length === 0) {
    throw new Error(`Invalid stop \`${record.stop}\` found`);
  }

  const routes = getRoutes({ route_id: record.route }, ['route_id']);

  if (routes.length === 0) {
    throw new Error(`Invalid route \`${record.route}\` found`);
  }

  const serviceDate = DateTime.fromFormat(record.calendar_date, 'yyyy-MM-dd HH:mm:ss.SSS').toFormat('yyyyMMdd');
  ...
  const trips = getTrips({ trip_id: record.trip_sn }, ['trip_id', 'direction_id']);

  if (trips.length === 0) {
    throw new Error(`No trip_id found for \`${record.trip_sn}\``);
  }

  const stopTime = findClosestStoptimeByTime({ tripId: record.trip_sn, stopId: record.stop }, gtfsTime);
