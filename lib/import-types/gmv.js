...
export const formatGMVLine = ({ record, lineNumber }) => {
  ...
  const serviceIds = getServiceIdsByDate(scheduledArrivalTime.toFormat('yyyyMMdd'));
  ...
  const stopTime = findClosestStoptimeByTime({ tripId, stopId: record.StopId }, arrivalTime.toFormat('HH:mm:ss'));
