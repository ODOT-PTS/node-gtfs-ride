...
export const formatWETALine = ({ record, lineNumber }) => {
  ...
  const destinationStopIds = getAllStationStopIds(routeInfo.destination_stop_id);

  const matchingTrips = [];

  const originStoptimes = db.prepare(
  ...
  for (const originStoptime of originStoptimes) {
    const destinationStoptimes = db.prepare(
      `SELECT * FROM stop_times WHERE stop_id IN (${destinationStopIds.map(() => '?').join(',')}) AND trip_id = ? AND stop_sequence > ? ORDER BY stop_sequence ASC`,
    ).all(
      ...destinationStopIds,
  ...
  if (matchingTrips.length === 0) {
    throw new Error(`Unable to find trip for stop_id \`${routeInfo.origin_stop_id}\` departure time \`${record.SchedDepart}\``);
  }

  if (matchingTrips.length > 1) {
    throw new Error(`Ambiguous trip match for stop_id \`${routeInfo.origin_stop_id}\` departure time \`${record.SchedDepart}\`. ${matchingTrips.length} possible trips found`);
  }
