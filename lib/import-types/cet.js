...
export const formatCETLine = ({ record, lineNumber }) => {
  const stops = getStops({ stop_name: record.RouteStop }, ['stop_id']);
  ...
  const routes = getRoutes(
    { route_short_name: Number.parseInt(record.Route, 10) },
    ['route_id']
  );
  ...
  if (serviceIds.length === 0) {
    throw new Error(
      `No service_ids found for ClientTime \`${record.ClientTime}\``
    );
  }
  ...
  const tripStopTime = findClosestTripByTime(
    ...
    DateTime.fromFormat(record.ClientTime, 'M/d/yyyy h:mm:ss a').toFormat(
      'HH:mm:ss'
    )
  );
