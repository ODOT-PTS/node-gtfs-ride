...
export const formatSwiftlyLine = ({ record, lineNumber }) => {
  ...
  const boardings = Number.parseInt(record.boardings, 10);
  const alightings = Number.parseInt(record.alightings, 10);
  const loadCount = Number.parseInt(record.occupancy_count, 10);
