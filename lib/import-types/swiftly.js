...
export const formatSwiftlyLine = async ({ record, lineNumber }) => {
  const trips = await getTrips({ trip_id: record.trip_id });
  const stops = await getStops({ stop_id: record.stop_id });

  const stoptimes = await getStoptimes({ trip_id: record.trip_id, stop_id: record.stop_id, departure_time: record.scheduled_time }, ['trip_id', 'stop_sequence']);
