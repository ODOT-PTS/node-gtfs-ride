...
const formatLTDLine = async ({ record, lineNumber }) => {
  ...
  }, DateTime.fromFormat(record.msg_time, 'HH:mm').toFormat('HH:mm:ss'));
...
const importLines = async (lines, model, config) => {
  ...
  config.log(`${agencyKey}: Importing - ${model.filenameBase}.txt - ${lineNumberStart + lines.length - 1} lines imported\r`, true);
};
...
const importFile = (filePath, config) => {
  ...
  return new Promise((resolve, reject) => {
    if (!fs.existsSync(filePath)) {
      ...
      return reject(new Error(`${agencyKey}: Importing - ${filePath} - No file found\r`));
    }

    config.log(`${agencyKey}: Importing - ${filePath}\r`);
