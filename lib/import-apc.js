...
const formatLTDLine = async ({ record, lineNumber }) => {
  ...
  const stopId = record.stop;
  const routeId = record.route;
  const queryParameters = [routeId, stopId, ...serviceIds];
  const directionId = convertIOtoDirection(record.dir);

  let directionIdWhereClause;
  if (directionId === null) {
    directionIdWhereClause = 'direction_id IS NULL';
  } else {
    directionIdWhereClause = 'direction_id = ?';
    queryParameters.push(directionId);
  }

  const tripStoptimes = await db.all(
    `SELECT * FROM trips INNER JOIN stop_times ON trips.trip_id = stop_times.trip_id WHERE route_id = ? AND stop_id = ? AND service_id IN (${serviceIds.map(() => '?').join(',')}) AND ${directionIdWhereClause} ORDER BY arrival_timestamp ASC`,
    queryParameters
  );

  if (tripStoptimes.length === 0) {
    throw new Error(`No trips found for route_id \`${routeId}\` and stop_id \`${stopId}\` and service_id \`${serviceIds.join(', ')}\` and direction_id \`${directionId}\``);
  }

  const tripStopTime = findClosestTripByTime(tripStoptimes, DateTime.fromFormat(record.msg_time, 'HH:mm').toFormat('HH:mm:ss'));
  ...
  return {
    ...
    record: {
      ...
      load_type: 1,
      service_date: serviceDate,
      service_departure_time: DateTime.fromFormat(record.msg_time, 'HH:mm').toFormat('HH:mm:ss'),
      source: 1
    }
  };
};
