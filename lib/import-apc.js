...
const formatLTDLine = async ({ record, lineNumber }) => {
  ...
  const trips = await gtfs.getTrips({ trip_id: record.trip_sn }, ['trip_id', 'direction_id']);

  if (trips.length === 0) {
    throw new Error(`No trip_id found for \`${record.trip_sn}\``);
  }

  const stopTimes = await gtfs.getStoptimes({ trip_id: record.trip_sn, stop_id: record.stop }, ['stop_sequence']);

  if (stopTimes.length === 0) {
    throw new Error(`No stop times found for trip_id \`${record.trip_sn}\` and stop_id \`${record.stop}\``);
  }
...
const formatRVTDLine = async ({ record, lineNumber }) => {
  ...
  const trips = await gtfs.getTrips({ trip_id: record.trip_id }, ['trip_id']);

  if (trips.length === 0) {
    throw new Error(`Invalid trip_id \`${record.trip_id}\` found`);
  }
...
const importFile = async (filePath, config) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Deduplicate lines from APC file before import
    neek.unique(inputStream, parser);
  })
    .catch(error => {
      throw error;
    });
