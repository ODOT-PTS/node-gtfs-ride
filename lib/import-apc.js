...
const formatCherriotsLine = async ({ record, lineNumber }) => {
  ...
  const gtfsTime = arrivalTime.toFormat('HH:mm:ss');
  const stopTime = await findClosestStoptimeByTime({ tripId: record.TripName, stopId: record.StopId }, gtfsTime);
  ...
  return {
    ...
    record: {
      ...
      record_use: 0,
      schedule_relationship: 0,
      boardings: parseInt(record.Ons, 10),
      alightings: parseInt(record.Offs, 10),
      load_count: parseInt(record.DeparturePassengers, 10),
      load_type: 1,
