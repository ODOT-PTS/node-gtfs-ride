...
const formatRVTDLine = async ({ record, lineNumber }) => {
  ...
  const stops = await gtfs.getStops({ stop_code: paddedStopCode }, ['stop_id']);
  ...
  const routes = await gtfs.getRoutes({ route_short_name: routeShortName }, ['route_id']);
