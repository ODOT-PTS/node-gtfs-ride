...
const convertToBoardAlights = (line, config) => {
  try {
    ...
    return boardAlightLines.map(line => cleanLine(line, boardAlightModel, config));
  } catch (error) {
...
const importLines = (lines, bar, config) => {
  ...
  try {
    db.prepare(`INSERT INTO ${model.filenameBase}(${fieldNames.join(', ')}) VALUES${placeholders.join(',')}`).run(...values);
  } catch (error) {
...
const createRideFeedInfo = () => {
  ...
  const fieldNames = model.schema.map(column => column.name);
  const firstBoardAlight = db.prepare('SELECT * FROM board_alight ORDER BY service_date ASC').get();
  const lastBoardAlight = db.prepare('SELECT * FROM board_alight ORDER BY service_date DESC').get();
  ...
  db.prepare(`INSERT INTO ${model.filenameBase}(${fieldNames.join(', ')}) VALUES${placeholders}`).run(...values);
};
