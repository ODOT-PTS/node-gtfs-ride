...
import { boardAlight, rideFeedInfo } from 'gtfs/models';
...
import { formatPassioLine } from './import-types/passio.js';
...
const getAPCType = (recordFieldNames) => {
  ...
  const passioRequiredFieldNames = [
    'Stop ID',
    'Trip ID',
    'Time point',
    'Count',
  ...
  if (
    passioRequiredFieldNames.every((fieldName) =>
      recordFieldNames.includes(fieldName),
    )
  ) {
    return 'passio';
...
const convertToBoardAlights = (line, config) => {
  try {
    ...
    switch (config.apcType) {
      case 'passio': {
        boardAlightLines.push(formatPassioLine(line));
    ...
    return boardAlightLines.map((line) => cleanLine(line, boardAlight, config));
  } catch (error) {
    if (
      config.ignoreMissingGTFSDates &&
      error?.cause?.reason === 'no-service-ids'
    ) {
      return [];
    }
    config.recordIssue({
      message: error.message,
      date: error.cause?.date,
      lineNumber: line.lineNumber,
      boardings: error.cause?.boardings,
      alightings: error.cause?.alightings,
    });
    return [];
  }
};
...
const importLines = (lines, bar, config) => {
  ...
  while (lines.length > 0) {
    ...
    for (const boardAlightLine of boardAlightLines) {
      ...
      formattedLines.push(formatLine(boardAlightLine, boardAlight));
    }
  }
  ...
  const fieldNames = boardAlight.schema.map((column) => column.name);
  const placeholders = [];
  const values = [];
  let lineNumberStart;
  ...
  try {
    db.prepare(
      `INSERT INTO ${boardAlight.filenameBase}(${fieldNames.join(
        ', ',
...
const importFile = async (filePath, config) => {
  if (!existsSync(filePath)) {
    config.recordIssue({
      message: `Importing - ${filePath} - No file found`,
    });
    ...
    parser.on('readable', () => {
      ...
      while ((record = parser.read())) {
        try {
          ...
          if (lines.length >= maxInsertVariables / boardAlight.schema.length) {
...
const createRideFeedInfo = () => {
  const db = openDb();
  const fieldNames = rideFeedInfo.schema.map((column) => column.name);
  ...
  const line = formatLine({ lineNumber: 1, record }, rideFeedInfo);
  const placeholders = `(${fieldNames.map(() => '?').join(', ')})`;
  const values = fieldNames.map((fieldName) => line.record[fieldName]);

  db.prepare(
    `INSERT INTO ${rideFeedInfo.filenameBase}(${fieldNames.join(
      ', ',
    )}) VALUES${placeholders}`,
  ).run(...values);
};
