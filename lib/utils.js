 ...
 */
exports.findClosestTripByTime = async ({ stopIds, serviceIds, routeId, directionId }, time) => {
  const db = gtfs.getDb();
  ...
  const tripStoptimes = await db.all(
    `SELECT * FROM trips INNER JOIN stop_times ON trips.trip_id = stop_times.trip_id WHERE route_id = ? AND stop_id IN (${stopIds.map(() => '?').join(',')}) AND service_id IN (${serviceIds.map(() => '?').join(',')}) AND ${directionIdWhereClause} ORDER BY arrival_timestamp ASC`,
    queryParameters
  );

  if (tripStoptimes.length === 0) {
    throw new Error(`No trips found for route_id \`${routeId}\` and stop_id \`${stopIds.join(', ')}\` and service_id \`${serviceIds.join(', ')}\` and direction_id \`${directionId}\``);
  }
