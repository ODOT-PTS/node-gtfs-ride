...
exports.findClosestTripByTime = async ({ stopIds, serviceIds, routeId, directionId }, gtfsTime) => {
  ...
  const tripStoptimes = await db.all(
    `SELECT * FROM trips INNER JOIN stop_times ON trips.trip_id = stop_times.trip_id WHERE route_id = ? AND stop_id IN (${stopIds.map(() => '?').join(',')}) AND service_id IN (${serviceIds.map(() => '?').join(',')}) AND ${directionIdWhereClause}`,
    queryParameters
  );
