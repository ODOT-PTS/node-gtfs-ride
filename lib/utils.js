...
async function estimateStoptimeArrivalAndDeparture(tripStoptime) {
  ...
  tripStoptime.arrival_timestamp = exports.calculateSecondsFromMidnight(estimatedTime.toFormat('HH:mm:ss'));
  tripStoptime.departure_time = estimatedTime.toFormat('HH:mm:ss');
  tripStoptime.departure_timestamp = exports.calculateSecondsFromMidnight(estimatedTime.toFormat('HH:mm:ss'));
 ...
 */
exports.findTripByFirstStoptime = async ({ routeId, stopId, stopSequence }, gtfsDate, gtfsTime) => {
  const db = gtfs.getDb();
  const serviceDate = DateTime.fromISO(gtfsDate).toFormat('yyyyMMdd');
  const serviceIds = await exports.getServiceIdsByDate(serviceDate);

  if (serviceIds.length === 0) {
    throw new Error(`No service_ids found for service_arrival_date \`${gtfsDate}\``);
  }

  const trips = await gtfs.getTrips({ route_id: routeId, service_Id: serviceIds }, ['trip_id', 'direction_id']);
 ...
 * stop_id.
 */
exports.getAllStationStopIds = async stopId => {
  ...
  if (stop.parent_station === '' || stop.parent_station === null) {
    ...
    const childStops = await gtfs.getStops({
