...
import { getDb, getStoptimes, getTrips, getCalendarDates, getStops } from 'gtfs';
 ...
 */
export async function findClosestTripByTime({ stopIds, serviceIds, routeId }, gtfsTime) {
  const db = getDb();
 ...
 */
export async function findClosestStoptimeByTime({ tripId, stopId }, gtfsTime) {
  const stoptimes = await getStoptimes({ trip_id: tripId, stop_id: stopId }, ['trip_id', 'stop_sequence', 'arrival_time', 'arrival_timestamp', 'departure_time', 'departure_timestamp']);
 ...
 */
export async function findTripByFirstStoptime({ routeId, stopId, stopSequence }, gtfsDate, gtfsTime) {
  const db = getDb();
  ...
  const trips = await getTrips({ route_id: routeId, service_Id: serviceIds }, ['trip_id', 'direction_id']);
  ...
  for (const trip of trips) {
    const lastStoptime = await db.get('SELECT trip_id, arrival_time FROM stop_times WHERE trip_id = ? ORDER BY stop_sequence ASC LIMIT 1', [trip.trip_id]);
 ...
 */
export async function getServiceIdsByDate(dateString) {
  const db = getDb();
  const days = [
    'monday',
    'tuesday',
  ...
  const calendarDates = await getCalendarDates({ date: dateString });
 ...
 * stop_id.
 */
export async function getAllStationStopIds(stopId) {
  ...
  if (stop.parent_station === '' || stop.parent_station === null) {
    ...
    const childStops = await getStops({
