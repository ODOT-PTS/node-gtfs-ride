 ...
 */
exports.findClosestTripByTime = async ({ stopIds, serviceIds, routeId, directionId }, gtfsTime) => {
  const db = gtfs.getDb();
  const queryParameters = [routeId, ...stopIds, ...serviceIds];

  let directionIdWhereClause = '';
  if (directionId === null) {
    directionIdWhereClause = 'AND direction_id IS NULL';
  } else if (directionId !== undefined) {
    directionIdWhereClause = 'AND direction_id = ?';
    queryParameters.push(directionId);
  }

  const tripStoptimes = await db.all(
    `SELECT * FROM trips INNER JOIN stop_times ON trips.trip_id = stop_times.trip_id WHERE route_id = ? AND stop_id IN (${stopIds.map(() => '?').join(',')}) AND service_id IN (${serviceIds.map(() => '?').join(',')}) ${directionIdWhereClause}`,
    queryParameters
  );

  if (tripStoptimes.length === 0) {
    let directionIdError = '';
    if (directionId !== undefined) {
      directionIdError = ` and direction_id \`${directionId}\``;
    }

    throw new Error(`No trips found for route_id \`${routeId}\` and stop_id \`${stopIds.join(', ')}\` and service_id \`${serviceIds.join(', ')}\`${directionIdError}`);
  }
...
/*
 * Convert alphabetical directcions "I" and "O" into numeric 1 and 0
 */
exports.convertIOtoDirection = direction => {
  if (direction === 'I') {
    return 1;
  }
