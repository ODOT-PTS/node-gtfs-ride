...
import { openDb, getStoptimes, getTrips, getCalendarDates, getStops } from 'gtfs';
...
function estimateStoptimeArrivalAndDeparture(tripStoptime) {
  ...
  const previousStoptime = db.prepare(
    'SELECT * FROM stop_times WHERE trip_id = ? AND stop_sequence < ? AND arrival_time IS NOT NULL ORDER BY stop_sequence ASC',
  ).get(tripStoptime.trip_id, tripStoptime.stop_sequence);
 ...
 */
export function findClosestTripByTime({ stopIds, serviceIds, routeId }, gtfsTime) {
  const db = openDb();
  const queryParameters = [routeId, ...stopIds, ...serviceIds];

  const tripStoptimes = db.prepare(
  ...
  const estimatedTripStoptimes = tripStoptimes.map(tripStoptime => estimateStoptimeArrivalAndDeparture(tripStoptime));
...
export function findClosestStoptimeByTime({ tripId, stopId }, gtfsTime) {
  const stoptimes = getStoptimes({ trip_id: tripId, stop_id: stopId }, ['trip_id', 'stop_sequence', 'arrival_time', 'arrival_timestamp', 'departure_time', 'departure_timestamp']);
  ...
  const estimatedTripStoptimes = stoptimes.map(tripStoptime => estimateStoptimeArrivalAndDeparture(tripStoptime));
  const sortedEstimatedTripStoptimes = sortBy(estimatedTripStoptimes, 'arrival_timestamp');
 ...
 */
export function findTripByFirstStoptime({ routeId, stopId, stopSequence }, gtfsDate, gtfsTime) {
  const db = openDb();
  const serviceDate = DateTime.fromISO(gtfsDate).toFormat('yyyyMMdd');
  const serviceIds = getServiceIdsByDate(serviceDate);

  if (serviceIds.length === 0) {
    throw new Error(`No service_ids found for service_arrival_date \`${gtfsDate}\``);
  }

  const trips = getTrips({ route_id: routeId, service_Id: serviceIds }, ['trip_id', 'direction_id']);

  if (trips.length === 0) {
    throw new Error(`No trip_ids found for route_id \`${routeId}\` on service_arrival_date \`${gtfsDate}\``);
  }

  let matchedTrip;
  for (const trip of trips) {
    const lastStoptime = db.prepare('SELECT trip_id, arrival_time FROM stop_times WHERE trip_id = ? ORDER BY stop_sequence ASC LIMIT 1').get(trip.trip_id);
  ...
  if (!matchedTrip) {
    throw new Error(`No trip_ids found for route_id \`${routeId}\` on service_arrival_date \`${gtfsDate}\` with trip end time of \`${gtfsTime}\``);
  }
...
export function getServiceIdsByDate(dateString) {
  ...
  const calendars = db.prepare(`SELECT service_id FROM calendar WHERE start_date <= ? AND end_date >= ? AND ${dayOfWeek} = 1`).all(dateString, dateString);
